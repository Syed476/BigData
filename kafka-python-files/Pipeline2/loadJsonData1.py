...
cities_df=df.withColumn("cities",F.explode(F.col("cities"))).select("cities.geonameid","cities.population")
cities_df.show()

cities_df.write.mode("overwrite").saveAsTable("mydb.city_table_updated1")
df_sql=spark.sql("select * from mydb.city_table_updated1")
